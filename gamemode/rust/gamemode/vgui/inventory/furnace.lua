...
function PANEL:Paint(w, h)
    ...
    draw.SimpleText("FURNACE", "RUST_Title", 10, 17.5, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
end

vgui.Register("RUST_Furnace", PANEL, "DPanel")

// ------------------------------------------------------------------

hook.Add("RUST_Update_Side_Panel_Slot", "RUST_Handle_Furnace_Update", function(create, invData, inv, slot, itemid, amount, itemData)
    if( IsValid(RUST.VGUI.BasePanel.rightPanel) && RUST.VGUI.BasePanel.rightPanel:GetName() == "RUST_Furnace" )then
        if( create )then
