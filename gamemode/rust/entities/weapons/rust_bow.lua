...
SWEP.Category = "Rust" --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep.
...
function SWEP:Ammo1()
	return RUST.GetItemAmountFromInv(self.Owner:GetInv(), "arrow") || 0
end

function SWEP:TakePrimaryAmmo(amount)
	if( SERVER )then
		self.Owner:RemoveItem(self.Owner:GetInv(), "arrow", 1)
	end
end
